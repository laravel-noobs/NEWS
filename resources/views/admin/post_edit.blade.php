...
app('navigator')
        ->activate('posts')
        ->set_page_heading('Sửa bài viết')
        ->set_breadcrumb('admin', 'posts', 'post_edit');
?>
...
@section('content')
    <div class="row">
        <form method="POST" action="{{ URL::action('PostsController@update', ['id' => $post->id]) }}">
            {{ csrf_field() }}
            <div class="col-lg-9">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>Sửa bài viết <strong>{{ $post->title }}</strong></h5>
                    ...
                    <div class="ibox-content">
                        <div class="form-horizontal">
                            <div class="form-group {{ count($errors->get('title')) > 0 ? 'has-error' : '' }}">
                                <div class="col-sm-12">
                                    <input placeholder="Nhập tiêu đề tại đây" id="title" name="title" value="{{ old('title', $post->title) }}" type="text" class="form-control">
                            ...
                            <div class="form-group">
                                <div class="col-md-12 "><textarea name="content" id="content" rows="10" cols="80">{{ old('content', $post->content) }}</textarea></div>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="form-group">
                                <div class="col-sm-12">
                                    <select name="tags[]" id="tags" class="tags form-control" multiple="multiple">
                                        ...
                                        @endforeach
                                        @foreach(old('existed_tags', $post->tags) as $tag)
